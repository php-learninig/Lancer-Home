...
class Mail
{
    ...
    private $smtp_log = '';
    /**
     * Принудительное отключение лога. Если получать лог отправки не нужно, лучше отключить, так как если идет отправка
     * писем по нескольким ресурсам, да еще с файлами, то потребление памяти будет большим По умолчанию находится в
    ...
    private $body_header = [];
    /**
     * Конвертация email адресов в национальных доменах. При включении будет работать и с национальными и с
     * международными доменами. Рекомендуется включать, если действительно есть необходимость, так как увеличивает
     * потребление памяти. По умолчанию выключена
     *
     * @var boolean
     */
     ...
     * @param boolean $idna_convert Нужна ли конвертация email из национальных доменов. При включении будет работать с
     *                              национальными и международными доменами одновременно. Рекоменуется включать только
     *                              если вы отправляете с нацонального домена или отправляете на национальные домены,
     *                              либо неизвестно, будет ли отправка на национальные домены или нет. Так как повышает
     *                              потребление памяти.
     */
     ...
     * @param string $alternative_text Альтернативный текст. Если письмо в html то здесь может быть текст, который
     *                                 будут показывать почтовики, которые не умеют отображать html
    ...
    public function Body($text, $text_html = "", $alternative_text = '', $resource = 'webi')
    {
        ...
        elseif(strlen($alternative_text) and $text_html == 'text/html') {
            ...
            $body .= "--ALT-"
                . $this->boundary
                . "--"; // И теперь закрывающий разделитель, показывает, что все части закончились
        }
        // в указанный ресурс добавляем
        $this->SubBody[$resource]['body'][0]
            = $body; // что получилось забарасываем в нулевой элемент массива частей. Это получилась первая часть тела письма. Возожно дальше будут формироваться еще части
    }
     ...
     * @param string $new_name_filename : реальное имя файла. если вдруг вставляется файл временный, то его имя будет
     *                                  не понятно каким
     * @param string $filetype          : MIME-тип файла. если не указан, попытается определить по ресширению, если не
     *                                  найдено будет application/octet-stream
     * @param string $disposition       по какому принципу вставляется файл. 'attachment' - файл прикрепится как
     *                                  отдельный файл, если ничего нет, тогда файл будет частью письма, например чтобы
     *                                  вставить изображение внутрь html текста, изображение не будет показываться как
     *                                  прикрепленный файл. При 'attachment' в почтовике должно быть видно
     *                                  прикрепленный файл и его можно скачать
...
// если тип файла не указан, пытаемся определить mime по расширению
     ...
     * @param string $resource Ресурс-для какого ресурса относится данная тема, далее, если для ресурса не будет
     *                         установлена тема, она возьмется из ресурса по умолчанию
     */
    public function Subject($subject, $resource = 'webi')
    {
        ...
        $this->headers[$resource]['Subject'] = "=?" . $this->charset . "?Q?" . str_replace(
                "+",
                "_",
                str_replace(
                    "%",
     ...
     * От кого
     *
     * @param string $from может быть имя и email через разделитель имя;asd@asde.ru либо просто email. From во всех
     *                     ресурсах одинаковый, его нельзя установить для каждого разный
     *
     * @return boolean
     */
    public function From($from)
    {
        ...
        if($this->checkAddress == true) {
            $this->headers['From'] = $this->CheckAdresses(
                $this->headers['From']
            ); // проверка с автоматической конвертацией национального домена. Поэтому если проверка прошла нормально, то возвращаются проверенные адреса и если нужна конвертация, то адреса будут отконвертированы и присваиваем новые адреса в переменную
        }
    }

    /**
     * Проверка валидности email. Возвращает ошибку или проверенные адреса, это удобно, если адреса были нациоанальные,
     * то вернуться они уже отконвертированные
     *
    ...
    private function CheckAdresses($aad)
    {
        if(is_array($aad)) // если массив адресов
        {
            foreach($aad as $key => $value) {
                ...
                if($this->idna_convert) {
                    $aad[$key]
                        = $value = $this->IDN->encode(
                        $value
                    ); // сохраняем преобразованный email обратно в массив, чтобы потом его возвратить
                }
        ...
        elseif(strlen($aad)) // либо если строка
        {
            ...
            if($this->idna_convert) {
                ...
                $aad = $this->IDN->encode(
                    $aad
                ); // сохраняем преобразованный email обратно в переменную, чтобы потом его возвратить
            }
     ...
     *
     * @param string $address Нельзя установить для каждого ресурса разный, отвечать можно будет всегда только на один
     *                        адрес
     *
     * @return boolean
     */
    public function ReplyTo($address)
    {
        ...
        // проверка адреса на валидность
        if($this->checkAddress == true) {
            $this->headers['Reply-To'] = $this->CheckAdresses(
    ...
    /**
     * Добавление заголовка для получения уведомления о прочтении. обратный адрес берется из "From" (или из "ReplyTo"
     * если указан) Данный параметр не актуален, так как многие почтовики игнорируют этот параметр, а некоторые
     * почтовые системы расчитывают письма с этим параметром как спам, так как этот параметр часто использовали спамеры
     * для проверки рабочих адресов
     */
    ...
    public function To($to, $resource = 'webi')
    {
        ...
        if(is_array($to)) {
            foreach($to as $key => $value) // перебираем массив и добавляем в массив для отправки через smtp
            {
                ...
                if(count($temp_mass) == 2) // если удалось разбить на два элемента
                {
                    $this->smtpsendto[$resource][$temp_mass[1]]
                        = $temp_mass[1]; // ключи и значения одинаковые, чтобы исключить дубли адресов
                    $this->names_email[$resource]['To'][$temp_mass[1]] = $temp_mass[0]; // имя первая часть
                    $this->sendto[$resource][] = $temp_mass[1];
                }
                else // и если имя не определено
                {
                    $this->smtpsendto[$resource][$value]
                        = $value; // ключи и значения одинаковые, чтобы исключить дубли адресов
        ...
        if($this->checkAddress == true) {
            $this->sendto[$resource] = $this->CheckAdresses(
     ...
     * @param string       $resource Ресурс-для которого будут установлены открытые копии. Если не установить для
     *                               ресурса, то подставляться ничего НЕ будет, из ресурса по умолчанию брать ничего НЕ
     *                               будет
     */
    public function Cc($cc, $resource = 'webi')
    {
        ...
        // проверка адресов на валидность
        if($this->checkAddress == true) {
            $this->acc[$resource] = $this->CheckAdresses(
                $this->acc[$resource]
            ); // проверка с автоматической конвертацией национального домена. Поэтому если проверка прошла нормально, то возвращаются проверенные адреса и если нужна конвертация, то адреса будут отконвертированы и присваиваем новые адреса в переменную
        }
    }
     ...
     * @param string       $resource Ресурс-для которого будут установлены скрытые копии. Если не установить для
     *                               ресурса, то подставляться ничего НЕ будет, из ресурса по умолчанию брать ничего НЕ
     *                               будет
     */
    public function Bcc($bcc, $resource = 'webi')
    {
        ...
        // проверка адреса на валидность
        if($this->checkAddress == true) {
            $this->abcc[$resource] = $this->CheckAdresses(
                $this->abcc[$resource]
            ); // проверка с автоматической конвертацией национального домена. Поэтому если проверка прошла нормально, то возвращаются проверенные адреса и если нужна конвертация, то адреса будут отконвертированы и присваиваем новые адреса в переменную
        }
    }
     ...
     * для отправки через защищенное соединение сервер нужно указывать с добавлением "ssl://" например так
     * "ssl://smtp.gmail.com"
     *
...
// разбиваем (FROM - от кого) на юзера и домен. юзер понадобится в приветсвии с сервом
            ...
            foreach($this->smtpsendto as $key_res => $value_res) {
                ...
                fputs(
                    $smtp_conn,
                    "MAIL FROM:<" . $this->headers['From'] . "> SIZE=" . strlen(
                        $this->ready_headers[$key_res] . "\r\n" . $this->body[$body_resource]
                    ) . "\r\n"
                );
                $this->add_log(
                    "Я: MAIL FROM:<" . $this->headers['From'] . "> SIZE=" . strlen(
                        $this->ready_headers[$key_res] . "\r\n" . $this->body[$body_resource]
                    ) . "\n"
                );
                $data = $this->get_data($smtp_conn) . "\n";
    ...
    public function BuildMail($resource = 'webi')
    {
        ...
        if(!isset($this->body[$resource_body])) {
            ...
            if(isset($this->SubBody[$resource_body]['mixed']) AND count($this->SubBody[$resource_body]['mixed'])) {
                ...
                $body = $body . "\r\n--MIX-" . $this->boundary . "\r\n" . $bodymix;
                $body = "Content-Type: multipart/mixed; boundary=MIX-"
                    . $this->boundary
                    . "\r\n\r\n"
                    . '--MIX-'
                    . $this->boundary
            ...
            $this->body_header[$resource_body]
                = $temp_mass[0]; // первый элемент и будет основной заголовок, его добавим в самый конец заголовков
            unset($temp_mass[0]); // теперь удаляем этот этот элемент из массива
            $this->body[$resource_body] = implode(
                "\r\n\r\n",
                $temp_mass
            ); // и формируем тело письма обратно но уже без основного заголовка и сразу добавляем его в основную переменную
        ...
        foreach($this->sendto[$resource] as $key => $value) {
            ...
            if(strlen($this->names_email[$resource]['To'][$value])) {
                $temp_mass[] = "=?" . $this->charset . "?Q?" . str_replace(
                        "+",
                        "_",
                        str_replace(
                            "%",
        ...
        $this->headers[$resource]['To'] = implode(
            ", ",
            $temp_mass
...
// если установлено подтверждение о доставке, берем адрес куда слать уведомление из адрес для ответа, либо из From
        ...
        if($this->smtp['on']) {
            ...
            $this->ready_headers[$resource] .= "Message-ID: <"
                . rand()
                . "."
                . $resource
                . date("YmjHis")
            ...
            while(list($hdr, $value) = each($new_mass_head)) {
                if($hdr == "From" and strlen($this->names_email['from'])) {
                    $this->ready_headers[$resource] .= $hdr . ": =?" . $this->charset . "?Q?" . str_replace(
                            "+",
                            "_",
                            str_replace(
                                "%",
